...
#1. На рисунке 1 пример использования команд с докером:
*docker --version (проверяет, установлен ли Docker на компьютере, и выводит его текущую версию)
*docker run hello-world (ищет на компьютере тестовый образ hello-world. Если его нет, скачивает его из интернета (с Docker Hub), запускает внутри контейнера, выводит приветственный текст и завершает работу. На рисунке 2 пример самого контейнера в докере)
